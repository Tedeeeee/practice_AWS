...
public class S3Uploader {
    ...
    private File convert(MultipartFile mf) throws IOException {
        ...
        throw new IllegalArgumentException(String.format("파일 변환에 실패하였습니다. %s", originalFileName));
    }
    ...
    private void removeNewFile(File targetFile) {
        if (targetFile.delete()) log.info("파일이 삭제 되었습니다");
        else log.info("파일 삭제를 실패하였습니다");
    }

    public void deleteFile(String fileName) {
        try {
            // URL 디코딩을 통해 원래의 파일 이름을 가져온다
            String decodedFileName = URLDecoder.decode(fileName, "UTF-8");
            log.info("Deleting file from S3 : " + decodedFileName);
            amazonS3.deleteObject(bucket, decodedFileName);
        } catch (UnsupportedEncodingException e) {
            log.error("디코딩 파일에 오류가 발생했습니다. : {}", e.getMessage());
        }
    }
